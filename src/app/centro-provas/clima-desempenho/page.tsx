...
function Barra({ label, valor, cor, n, velMax }: { label: string; valor: number | null; cor: string; n: number; velMax: number }) {
  return (
        ...
        {ok.length >= 2 && (
          <section style={{ ...T.card, borderColor: `${T.gold}55`, background: `${T.gold}0d` }}>
            ...
            <div style={{ display: "grid", gap: 8 }}>
              <Barra label="🟢 Vento a favor" valor={media(favor)} cor={T.green} n={favor.length}  velMax={velMax} />
              <Barra label="🟡 Vento lateral" valor={media(lateral)} cor="#fbbf24" n={lateral.length}  velMax={velMax} />
              <Barra label="🔴 Vento contra" valor={media(contra)} cor={T.red} n={contra.length}  velMax={velMax} />
              <Barra label="🌧️ Dias com chuva" valor={media(chuva)} cor="#55a3ff" n={chuva.length}  velMax={velMax} />
              <Barra label="☀️ Dias secos" valor={media(seco)} cor={T.gold} n={seco.length}  velMax={velMax} />
            </div>
