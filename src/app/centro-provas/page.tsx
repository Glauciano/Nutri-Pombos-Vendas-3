...
export default function CentroProvas() {
  ...
  const provaAtual = provas.find(p => p.num === provaNum);
  ...
  if (aba === "calendario") return <Shell><Calendario passadas={passadas} proximaNum={proxima?.num} provas={provas} onBack={() => setAba("hub")} onOpen={num => { setProvaNum(num); setAba("detalhe"); }} /></Shell>;
  ...
  return <Shell>
    ...
    <ModuleGroup title="🌾 Nutrição e Saúde" subtitle="Receitas, suplementos, prevenção e rotina">
      ...
      <Link href="/centro-provas/recuperacao"><Module icon="🔋" title="Recuperação Pós-Prova" description="Limpeza de ácido láctico" color="#22c55e" /></Link>
