...
type Treino={id:string;data:string;distancia:number;local:string;tempoMin:number;clima:string;observacoes:string};
type Form={data:string;distancia:number;local:string;horas:number;minutos:number;segundos:number;clima:string;observacoes:string};
const KEY="nutripombos-treinos-v1";
const CLIMAS=["Ensolarado","Nublado","Chuvoso","Vento leve","Vento forte","Frio","Quente"];
const vazio=():Form=>({data:new Date().toISOString().slice(0,10),distancia:50,local:"",horas:0,minutos:45,segundos:0,clima:"Ensolarado",observacoes:""});
function read():Treino[]{try{const v=JSON.parse(localStorage.getItem(KEY)||"[]");return Array.isArray(v)?v:[]}catch{return[]}}
function paraForm(t:Treino):Form{const total=Math.round(t.tempoMin*60);return{data:t.data,distancia:t.distancia,local:t.local,horas:Math.floor(total/3600),minutos:Math.floor(total%3600/60),segundos:total%60,clima:t.clima,observacoes:t.observacoes}}
function totalSeg(f:Form){return Math.max(0,f.horas*3600+f.minutos*60+f.segundos)}
function hms(seg:number){return[Math.floor(seg/3600),Math.floor(seg%3600/60),seg%60].map(v=>String(v).padStart(2,"0")).join(":")}

export default function Treinamentos(){
  const[treinos,setTreinos]=useState<Treino[]>([]);const[ready,setReady]=useState(false);const[show,setShow]=useState(false);const[editId,setEditId]=useState<string|null>(null);const[form,setForm]=useState<Form>(vazio());
  useEffect(()=>{setTreinos(read());setReady(true)},[]);useEffect(()=>{if(ready)localStorage.setItem(KEY,JSON.stringify(treinos))},[treinos,ready]);
  const segundos=totalSeg(form),horas=segundos/3600,velKmh=horas>0?form.distancia/horas:0,velMmin=segundos>0?form.distancia*1000/(segundos/60):0;
  const totalKm=treinos.reduce((s,t)=>s+t.distancia,0),maior=treinos.length?Math.max(...treinos.map(t=>t.distancia)):0,media=treinos.length?Math.round(totalKm/treinos.length):0;
  function abrir(t?:Treino){setForm(t?paraForm(t):vazio());setEditId(t?.id||null);setShow(true)}
  function salvar(){if(!form.local.trim()){alert("Informe o local.");return}if(segundos<=0){alert("Informe uma duração válida.");return}const dados={data:form.data,distancia:form.distancia,local:form.local,tempoMin:segundos/60,clima:form.clima,observacoes:form.observacoes};setTreinos(v=>(editId?v.map(t=>t.id===editId?{...t,...dados}:t):[...v,{id:crypto.randomUUID(),...dados}]).sort((a,b)=>a.data.localeCompare(b.data)));setShow(false);setEditId(null)}
  return <main style={{minHeight:"100vh",background:T.bg,color:T.white,padding:"18px 12px 50px"}}><div style={{maxWidth:760,margin:"0 auto"}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"start",gap:10,marginBottom:20}}><div><Link href="/centro-provas" style={{...T.small,textDecoration:"none"}}>← Centro de Provas</Link><h1 style={{...T.h1,marginTop:9}}>🏋️ Treinamentos</h1><p style={{...T.small,marginTop:4}}>Histórico, duração e evolução dos treinos</p></div><button onClick={()=>abrir()} style={T.btnSm}>+ Treino</button></div>
    <div className="training-stats" style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8,marginBottom:12}}>{[["Treinos",treinos.length],["Km totais",`${totalKm}km`],["Maior dist.",`${maior}km`]].map(([l,v])=><div key={l} style={{...T.card,margin:0,padding:11,textAlign:"center"}}><div style={T.small}>{l}</div><b style={{display:"block",fontSize:20,color:T.gold,marginTop:4}}>{v}</b></div>)}</div>
